...
## 目前小米、华为等手机发现的bug
1. 小米华为（部分手机）Rationale不支持，系统记录错误。
2. 小米华为（部分手机）同意授权，但是底层还是没有授权，回调成功后还是没有权限。
3. 小米华为（部分手机）同意授权，但是底层回调了我们失败。
4. AndPermission已经最大程度兼容并友好提示用户了，建议在发布的时候在>=6.0和<6.0的手机上测试，其它只能等待厂家修复。
